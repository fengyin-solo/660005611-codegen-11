...
import type { ReportFilter, StageDef, SummaryResult, DetailsResult, TrendResult } from '@/types'

// 汇总 / 明细 / 趋势三处共用同一组查询参数，保证口径一致：
// 时间按执行开始时间过滤，环节按 stageIds 过滤（空 = 全部）。
function commonParams(f: ReportFilter) {
...
export async function fetchSummary(f: ReportFilter): Promise<SummaryResult> {
  const { data } = await axios.get('/api/reports/summary', { params: commonParams(f) })
  return data
}

export async function fetchDetails(f: ReportFilter, page: number, pageSize: number): Promise<DetailsResult> {
...
export async function fetchTrend(f: ReportFilter, limit: number): Promise<TrendResult> {
